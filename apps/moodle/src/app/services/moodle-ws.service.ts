...
export class MoodleWsService {
  ...
  login(userLoginDto: UserLoginDto): Observable<LoginResponse> {
    ...
    return this.httpClient.post<LoginResponse>(loginRequest.url, loginRequest.formData, {params: loginRequest.queryParams});
  }

  registerUser(createUserDto: CreateUserDto): Observable<CoreUserCreateUsersResponse> {
    let registerRequest = new CoreUserCreateUsersRequest(createUserDto);
    return this.httpClient.post<CoreUserCreateUsersResponse>(registerRequest.url, registerRequest.formData, {params: registerRequest.queryParams});
  }
  ...
  getUserDetail(token: string): Observable<CoreWebserviceGetSiteInfoResponse> {
    ...
    return this.httpClient.post<CoreWebserviceGetSiteInfoResponse>(getUserDetailRequest.url, getUserDetailRequest.formData, {params: getUserDetailRequest.queryParams});
  }

  getCourseCategories(token: string): Observable<CoreCourseGetCategoriesResponse> {
    let getCourseCategories = new CoreCourseGetCategoriesRequest(token);
    return this.httpClient.post<CoreCourseGetCategoriesResponse>(getCourseCategories.url, getCourseCategories.formData, {params: getCourseCategories.queryParams});
  }

  getCourseByField(getCourseByFieldDto: GetCourseByFieldDto): Observable<CoreCourseGetCoursesByFieldResponse> {
    let getCourseByFieldRequest = new CoreCourseGetCoursesByFieldRequest(getCourseByFieldDto);
    return this.httpClient.post<CoreCourseGetCoursesByFieldResponse>(getCourseByFieldRequest.url, getCourseByFieldRequest.formData, {params: getCourseByFieldRequest.queryParams});
  }

  selfEnrolCourse(selfEnrolCourseDto: SelfEnrolCourseDto): Observable<EnrolSelfEnrolUserResponse> {
    let selfEnrolRequest = new EnrolSelfEnrolUserRequest(selfEnrolCourseDto);
    return this.httpClient.post<EnrolSelfEnrolUserResponse>(selfEnrolRequest.url, selfEnrolRequest.formData, {params: selfEnrolRequest.queryParams});
  }
}
