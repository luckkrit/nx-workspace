...
export class EnrolSelfEnrolUserRequest extends BaseRequest {
  constructor(selfEnrolCourseDto: SelfEnrolCourseDto) {
    super();
    this.url += environment.apiPath;
    this.appendFormData(MoodleWsFormDataParamters.WS_COURSE_ID, selfEnrolCourseDto.courseId);
    this.appendQueryParams({
      [MoodleWsQueryParameters.WS_FUNCTION]: [MoodleWsFunctions.ENROL_SELF_ENROL_USER],
      [MoodleWsQueryParameters.WS_TOKEN]: selfEnrolCourseDto.token
    });
  }
}
